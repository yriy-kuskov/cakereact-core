[⬅️Назад](./README.md)
### 🛡️ 2. Guardrail для BaseEntity

Файл: `tests/ai_guardrails/BaseEntity.integrity.test.js`

Здесь мы проверяем:

1. Что сущность — это `Proxy` (магия геттеров/сеттеров).
2. Что механизм `dirty` (отслеживание изменений) работает и не был удален.
3. Что методы `clean`, `toArray`, `isNew` на месте.
