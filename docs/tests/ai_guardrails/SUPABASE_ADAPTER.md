[⬅️Назад](./README.md)
### 🛡️ 1. Guardrail для SupabaseAdapter

Файл: `tests/ai_guardrails/SupabaseAdapter.integrity.test.js`

Здесь мы проверяем:

1. Наличие всех обязательных методов (включая вспомогательный `_buildSelectQuery`).
2. Что `findById` использует оптимизированный метод `.single()`.
3. Что `_buildSelectQuery` вызывается внутри `find`.
